...
const ModalWrapper: React.FC<{ children: React.ReactNode, onClose: () => void }> = ({ children, onClose }) => (
  ...
  >
    <div 
      className="bg-surface-light dark:bg-surface-dark border border-white/20 dark:border-white/10 rounded-3xl shadow-2xl shadow-brand-dark/20 dark:shadow-black/30 p-6 sm:p-8 text-text-light dark:text-text-dark max-w-2xl w-full animate-pop-in"
  ...
  return (
    <ModalWrapper onClose={onClose}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
       ...
       <button
        onClick={onClose}
        className="w-full mt-6 px-6 py-3 bg-accent text-white font-bold rounded-full hover:bg-accent-hover transition-all duration-200 ease-in-out active:scale-95 text-lg"
       >
        {t('close')}
      </button>
    </ModalWrapper>
